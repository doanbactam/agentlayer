...
export class ContextStore {
  ...
  init(): void {
    ...
    `);
    this.db.run(
      "CREATE INDEX IF NOT EXISTS idx_entries_file ON context_entries(file_path)",
    );
    this.db.run(
      "CREATE INDEX IF NOT EXISTS idx_entries_type ON context_entries(type)",
    );
    this.db.run(
      "CREATE INDEX IF NOT EXISTS idx_entries_scope ON context_entries(scope)",
    );
    this.db.run(
      "CREATE INDEX IF NOT EXISTS idx_entries_priority ON context_entries(priority)",
    );
    this.db.run(
      "CREATE INDEX IF NOT EXISTS idx_behaviors_agent ON behavior_log(agent_type)",
    );
    this.db.run(
      "CREATE INDEX IF NOT EXISTS idx_behaviors_file ON behavior_log(file_path)",
    );
  }
  ...
  addRule(rule: Rule): void {
    ...
    const priorityLabel =
      rule.priority >= PRIORITY_THRESHOLDS.critical
        ? "critical"
        : rule.priority >= PRIORITY_THRESHOLDS.high
          ? "high"
          : rule.priority >= PRIORITY_THRESHOLDS.normal
            ? "normal"
            : "low";
    this.db.run(
  ...
  replaceRules(source: string, rules: Rule[]): void {
    const insert = this.db.transaction(() => {
      this.db.run(
        "DELETE FROM context_entries WHERE type = 'rule' AND source = ?",
        [source],
      );
  ...
  getHealth(): StoreHealth {
    const entriesQuery = this.db.query(
      "SELECT COUNT(*) as c FROM context_entries",
    );
    const entries = entriesQuery.get() as { c: number };

    const staleQuery = this.db.query(
      "SELECT COUNT(*) as c FROM context_entries WHERE updated_at < datetime('now', '-30 days')",
    );
...
function rowToEntry(row: Row): ContextEntry {
  ...
  return {
    ...
    annotations:
      row.type === "annotation" ? [parsed as unknown as Annotation] : [],
    behaviors:
      row.type === "behavior" ? [parsed as unknown as BehaviorEntry] : [],
