...
const standardScripts = new Set([
  "start", "build", "test", "lint", "format", "dev", "preview",
...
function pat(path: string, pattern: string, reason: string, line?: number, snippet?: string): NonInferablePattern {
...
async function checkPackageJson(root: string): Promise<NonInferablePattern[]> {
  ...
  for (const [name, cmd] of Object.entries(scripts)) {
    ...
    if (prefix && standardScripts.has(prefix[2])) {
      out.push(pat("package.json", `lifecycle:${name}`, `Lifecycle hook wrapping "${prefix[2]}" — runs implicitly`))
      continue
    }
    out.push(pat("package.json", `custom-script:${name}`, `Custom script: ${String(cmd).slice(0, 60)}`))
  }

  // package manager
  if (pkg.packageManager) {
    out.push(pat("package.json", "package-manager", `Locked package manager: ${pkg.packageManager}`))
  }
  // bun lockfile
  if (existsSync(`${root}/bun.lockb`) || existsSync(`${root}/bun.lock`)) {
    out.push(pat("bun.lock", "package-manager", "Uses Bun as package manager (not npm/yarn/pnpm)"))
  }
  ...
  for (const field of interestingFields) {
    ...
    out.push(pat("package.json", `hidden-dep:${field}`, `${keys.length} ${field}: ${keys.slice(0, 5).join(", ")}${keys.length > 5 ? " ..." : ""}`))
  }
  ...
  for (const [name] of Object.entries(deps ?? {})) {
    if (nativeHints.some(h => name.toLowerCase().includes(h))) {
      out.push(pat("package.json", "native-module", `Native module dependency: ${name}`))
    }
  }
...
async function checkEnvFiles(root: string): Promise<NonInferablePattern[]> {
  ...
  for (const f of envFiles) {
    ...
    const keys = raw.split("\n")
      ...
      .filter(Boolean)
    if (keys.length > 0) {
      out.push(pat(f, "env-required", `Requires env vars: ${keys.slice(0, 6).join(", ")}${keys.length > 6 ? " ..." : ""}`))
    }
  }
  return out
}
...
const textExts = new Set([".ts", ".tsx", ".js", ".jsx", ".mjs", ".cjs", ".py", ".rb", ".go", ".rs", ".java", ".vue", ".svelte"])
...
async function scanFileContent(root: string, file: FileInfo): Promise<NonInferablePattern[]> {
  ...
  for (const [re, pattern, reason] of contentPatterns) {
    for (let i = 0; i < lines.length; i++) {
      if (re.test(lines[i])) {
        out.push(pat(file.path, pattern, reason, i + 1, lines[i].trim().slice(0, 80)))
  ...
  if (/(^|\/)index\.(ts|js)$/.test(file.path)) {
    const reexportCount = lines.filter(l => /^\s*export \* from/.test(l) || /^\s*export \{.*\} from/.test(l)).length
    if (reexportCount >= 3) {
      out.push(pat(file.path, "barrel-export", `Barrel file with ${reexportCount} re-exports`, undefined, undefined))
    }
  }
...
export async function detectPatterns(root: string, files: FileInfo[]): Promise<NonInferablePattern[]> {
  const out: NonInferablePattern[] = []
  ...
  for (let i = 0; i < toScan.length; i += chunkSize) {
    ...
    const results = await Promise.all(chunk.map(f => scanFileContent(root, f)))
