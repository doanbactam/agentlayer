...
export async function insights(opts: { json?: boolean }): Promise<void> {
  ...
  const totalRow = db.query("SELECT COUNT(*) as c FROM behavior_log").get() as {
    c: number;
  };
  ...
  const successRow = db
    .query("SELECT SUM(success) as s FROM behavior_log")
...
function renderDashboard(r: InsightResult): void {
  ...
  if (r.hotspots.length > 0) {
    ...
    for (const h of r.hotspots) {
      ...
      console.log(
        `  ${chalk.red("\u26A0")} ${path}  ${chalk.red(`${h.failures} ${label}`)} ${chalk.dim("-- consider annotating")}`,
      );
    }
    console.log();
  }
  ...
  const rateColor =
    r.successRate >= 0.8
      ...
      : r.successRate >= 0.5
        ...
        : chalk.red;
  console.log(
    `  Overall success rate: ${rateColor(pct(r.successRate))} (${r.totalSuccesses}/${r.totalEvents})`,
  );
  console.log();
}
