...
export async function overlay(opts: OverlayOptions = {}) {
  ...
  const contextMap = new Map<string, { rules: number; annotations: number; behaviors: number; hasFailures: boolean }>()
  ...
  try {
    ...
    for (const entry of allEntries) {
      ...
      const existing = contextMap.get(entry.path) || { rules: 0, annotations: 0, behaviors: 0, hasFailures: false }
    ...
    for (const b of failedBehaviors) {
      if (b.path) {
        const existing = contextMap.get(b.path) || { rules: 0, annotations: 0, behaviors: 0, hasFailures: false }
  ...
  for (const file of files) {
    ...
    const context = contextMap.get(file.path) || { rules: 0, annotations: 0, behaviors: 0, hasFailures: false }

    // Filter by directory if specified
    if (opts.dir) {
      const dirPattern = opts.dir.replace(/\\/g, "/").replace(/^\.\//, "").replace(/\/$/, "")
      if (!file.path.startsWith(dirPattern + "/") && file.path !== dirPattern) {
        continue
      }
    }

    const hasContext = context.rules > 0 || context.annotations > 0 || context.behaviors > 0
  ...
  gaps.sort((a, b) => {
    if (a.classification === "source" && b.classification !== "source") return -1
    if (a.classification !== "source" && b.classification === "source") return 1
  ...
  console.log(
    chalk.gray(`  Found ${chalk.white(gaps.length)} file${gaps.length === 1 ? "" : ""}${opts.unclassified ? " without context" : ""}:`)
  )
  console.log("")
  ...
  try {
    ...
    while (gaps.length > 0) {
      ...
      const selection = await question(`  Select a file to annotate (1-${gaps.length}) or 'q' to quit: `)

      if (selection.trim().toLowerCase() === "q") {
        console.log("")
        console.log(chalk.gray(`  Session ended. ${chalk.white(annotatedCount)} annotated, ${chalk.white(skippedCount)} skipped.`))
        console.log("")
        break
      }
    ...
    if (gaps.length === 0 && (annotatedCount > 0 || skippedCount > 0)) {
      ...
      console.log(chalk.gray(`  Session: ${chalk.white(annotatedCount)} annotated, ${chalk.white(skippedCount)} skipped.`))
      console.log("")
    }
  } finally {
    rl.close()
...
function displayFileList(gaps: FileGap[], annotatedCount: number) {
  console.log(chalk.dim("─").repeat(54))
  if (annotatedCount > 0) {
    console.log(chalk.gray(`  Progress: ${chalk.white(annotatedCount)} annotated this session`))
    console.log("")
  }
  ...
  for (let i = 0; i < displayCount; i++) {
    ...
    console.log(`  ${num} ${clsColor(gap.path.padEnd(32))} ${chalk.gray(sizeStr)}${contextStr}`)
  }
...
async function annotateFile(
  ...
  // File info
  console.log(`  Classification: ${classificationColor(gap.classification)(gap.classification)}`)
  console.log(`  Size:           ${chalk.gray(formatSize(gap.size))}`)

  const hasContext = gap.rules > 0 || gap.annotations > 0 || gap.behaviors > 0
  console.log(`  Existing context: ${hasContext ? chalk.yellow("partial") : chalk.gray("none")}`)
  ...
  // Get annotation text
  const annotationText = await question("  Enter annotation (or 's' to skip, 'q' to quit):\n  > ")
  ...
  // Get priority
  const priorityInput = await question(`  Priority ${chalk.gray("(critical/high/normal/low)")} [normal]: `)
  ...
  console.log("")
  console.log(chalk.green("  Annotation saved for") + " " + chalk.cyan(gap.path))
  console.log("")

  return "annotated"
}

async function detectFilePatterns(filePath: string, cwd: string): Promise<string[]> {
  ...
  try {
    ...
    if (/require\(['"]fs['"]\)/.test(content) || /from\s+['"]node:fs['"]/.test(content) || /import.*['"]fs['"]/.test(content)) {
    ...
    if (/process\.env/.test(content)) {
      ...
      if (envMatches) {
        const vars = [...new Set(envMatches.map(m => m.replace("process.env.", "")))].slice(0, 4)
    ...
    if (/export\s+(default\s+)?function|export\s+(default\s+)?class|export\s+const/.test(content)) {
      const exportCount = (content.match(/export\s+(default\s+)?(function|class|const)/g) || []).length
      if (exportCount >= 3) {
