...
export async function pull(opts: { force?: boolean }): Promise<void> {
  ...
  if (parsed.validCount === 0) {
    console.log(chalk.red("Context JSONL contains no valid entries. Import aborted."))
    return
  }

  if (parsed.invalidCount > 0) {
    console.log(chalk.yellow(`Skipped ${parsed.invalidCount} malformed JSONL line${parsed.invalidCount !== 1 ? "s" : ""}.`))
  }

  const store = new ContextStore(root)
  const db = store.getDb()

  const before = (db.query("SELECT COUNT(*) as c FROM context_entries").get() as { c: number }).c

  const incomingIds = parsed.ids

  const existingIds = new Set(
    (db.query("SELECT id FROM context_entries").all() as { id: number }[]).map((r) => r.id)
  )
  ...
  const after = (db.query("SELECT COUNT(*) as c FROM context_entries").get() as { c: number }).c
  imported = opts.force ? after : after - before
  store.close()

  console.log(
    chalk.green(`Pulled ${imported > 0 ? imported : 0} new entries, ${skipped} existing, ${removed} removed`)
  )
