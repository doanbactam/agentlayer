...
export async function status() {
  ...
  if (fs.existsSync(dbPath)) {
    ...
    try {
      ...
      const filesWithRules = new Set(rules.map((r) => r.path).filter(Boolean));
  ...
  // Context coverage
  const coverageColor =
    coverage >= 80 ? chalk.green : coverage >= 50 ? chalk.yellow : chalk.red;

  console.log(`  Context coverage: ${coverageColor(coverage + "%")}`);
  console.log(`  ${renderBar(coverage, coverageColor)}`);
  console.log("");
  ...
  if (health) {
    if (health.staleEntries > 0) {
      console.log(chalk.yellow(`  Stale entries:  ${health.staleEntries}`));
    }
    if (health.orphanedRules > 0) {
      console.log(chalk.yellow(`  Orphaned rules: ${health.orphanedRules}`));
    }
  ...
  if (fullDumpSize > 0 && injectedSize > 0 && injectedSize < fullDumpSize) {
    const reduction = Math.round(
      ((fullDumpSize - injectedSize) / fullDumpSize) * 100,
    );
    ...
    console.log(
      chalk.gray(
        `    Full context dump: ${fullDumpSize.toLocaleString()} chars (~${fullTokens.toLocaleString()} tokens)`,
      ),
    );
    console.log(
      chalk.gray(
        `    Agentmind inject: ${injectedSize.toLocaleString()} chars (~${injectTokens.toLocaleString()} tokens)`,
      ),
    );
    console.log("");
  }
  ...
  if (fs.existsSync(jsonlPath)) {
    ...
    console.log(
      chalk.gray(
        `  JSONL: ${formatBytes(stat.size)} at .agentmind/context.jsonl`,
      ),
    );
  }
  console.log("");
...
function renderBar(percentage: number, color: (s: string) => string): string {
  const width = 30;
...
function estimateInjectedSize(
  rules: ContextEntry[],
  annotations: ContextEntry[],
): number {
