...
export async function claim(opts: {
  ...
  if (conflicts.length > 0) {
    ...
    for (const c of conflicts) {
      console.log(
        chalk.yellow(`  ${c.filePath} — also edited by ${c.agents.join(", ")}`),
      );
    }
  }
}
...
export async function bridgeStatus(): Promise<void> {
  ...
  for (const a of agents) {
    ...
    const files =
      a.editingFiles.length > 0
        ? a.editingFiles.map((f) => `\n    ${chalk.gray(f)}`).join("")
        : "\n    (no files claimed)";
    console.log(
      `  ${chalk.bold(a.agentId)}  ${chalk.gray(a.tool)}  ${chalk.gray(`${ago}s ago`)}`,
    );
    console.log(files);
  }
...
export async function conflicts(): Promise<void> {
  ...
  for (const c of all) {
    const label =
      c.severity === "critical" ? chalk.red("CRITICAL") : chalk.yellow("WARN");
    console.log(`  ${label}  ${c.filePath}`);
    console.log(chalk.gray(`    agents: ${c.conflictingAgents.join(", ")}`));
  }
  console.log("");
