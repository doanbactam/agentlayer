...
export async function push(opts: {
  ...
  console.log(
    chalk.green(`Pushed ${lineCount} entries to .agentmind/context.jsonl`),
  );
  ...
  try {
    const status = execSync("git status --porcelain .agentmind/context.jsonl", {
      cwd: root,
      encoding: "utf-8",
    }).trim();
    if (!status) {
    ...
    execFileSync("git", ["add", ".agentmind/context.jsonl"], {
      cwd: root,
      stdio: "pipe",
    });
    ...
    console.log(
      chalk.dim("JSONL file is updated locally but was not committed."),
    );
    return;
  }
