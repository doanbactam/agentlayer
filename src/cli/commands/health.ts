...
function computeDirCoverage(
  ...
  const coveredPaths = new Set(
    rows.map((row) => row.file_path.replace(/\\/g, "/")),
  );
  const dirMap = new Map<
    string,
    { total: Set<string>; covered: Set<string> }
  >();
...
function computeClassifications(
  ...
  const allTypes: FileClassification[] = [
    "source",
    "data",
    "config",
    "test",
...
function computeBehavior(
  ...
  return {
    ...
    totalSuccesses,
    totalEvents,
    mostEdited: mostEditedRow
      ? { path: mostEditedRow.file_path, edits: mostEditedRow.edits }
      : null,
    mostFailed: mostFailedRow
      ? { path: mostFailedRow.file_path, failures: mostFailedRow.failures }
      : null,
  };
}
...
function renderCoverage(dirs: DirCoverage[]): void {
  ...
  for (const d of dirs) {
    ...
    console.log(
      `  ${chalk.dim(dirLabel)}  ${colorFn(bar)}  ${colorFn(pctLabel)}  ${chalk.dim(fileLabel)}`,
    );
  }
}
...
function renderClassifications(
  ...
  for (const c of counts) {
    ...
    console.log(
      `  ${chalk.dim(label)}  ${chalk.cyan(bar)}  ${chalk.bold(numLabel)}  ${chalk.dim(pctLabel)}`,
    );
  }
}
...
function renderStaleness(stale: StaleEntry[]): void {
  ...
  for (const s of stale) {
    ...
    console.log(
      `  ${chalk.yellow("\u26A0")} ${displayPath.padEnd(maxPathLen)}  ${chalk.dim("last updated " + daysLabel)}`,
    );
  }
}
...
function renderBehavior(behavior: BehaviorSummary | null): void {
  ...
  console.log(
    `  Success rate: ${colorFn(pct + "%")} ${chalk.dim(`(${behavior.totalSuccesses}/${behavior.totalEvents})`)}`,
  );

  if (behavior.mostEdited) {
    console.log(
      `  Most edited:  ${chalk.white(behavior.mostEdited.path)} ${chalk.dim(`(${behavior.mostEdited.edits} edit${behavior.mostEdited.edits !== 1 ? "s" : ""})`)}`,
    );
  }

  if (behavior.mostFailed) {
    console.log(
      `  Most failed:  ${chalk.red(behavior.mostFailed.path)} ${chalk.dim(`(${behavior.mostFailed.failures} failure${behavior.mostFailed.failures !== 1 ? "s" : ""})`)}`,
    );
  }
...
function renderRecommendations(recs: Recommendation[]): void {
  ...
  if (recs.length === 0) {
    console.log(
      chalk.green("  Everything looks good! No immediate actions needed."),
    );
    return;
  }
...
function extractDir(filePath: string): string {
  ...
  const dir =
    secondSlash !== -1
      ? normalized.slice(0, secondSlash)
