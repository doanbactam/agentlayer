  ...
  try {
    ...
    if (raw.length === 0) {
      const totalRow = store
        .getDb()
        .query("SELECT COUNT(*) as c FROM behavior_log")
        .get() as { c: number };
      if (totalRow.c === 0) {
        console.log(chalk.dim("No behavior data recorded yet."));
      } else {
        console.log(
          chalk.dim("No actionable patterns found in behavior data."),
        );
      }
      return;
    }
    ...
    const totalBehaviors = (
      store.getDb().query("SELECT COUNT(*) as c FROM behavior_log").get() as {
        c: number;
      }
    ).c;
    ...
    console.log(
      chalk.bold(`  Learned rules `) +
        chalk.green(`${high.length} high confidence`) +
        chalk.dim(", ") +
    ...
    for (const gr of [...high, ...medium]) {
      const learned = raw.find(
        (l) => l.filePath === gr.filePath && l.pattern === gr.rule.pattern,
      );
      if (!learned) continue;
    ...
    if (opts.apply) {
      ...
      console.log(
        chalk.green(
          `  Applied ${applied} learned rule${applied === 1 ? "" : "s"} to store.`,
        ),
      );
    } else {
      console.log(
        chalk.dim(
          "  Run `agentmind learn --apply` to add high-confidence rules.",
        ),
      );
      console.log(
        chalk.dim("  Run `agentmind learn --apply --force` to add all rules."),
      );
    }
...
function formatEvidence(rule: LearnedRule): string {
  ...
  if (rule.pattern === "frequent-failure") {
    parts.push(
      `${rule.evidence.length} failure${rule.evidence.length !== 1 ? "s" : ""}`,
    );
  } else if (rule.pattern === "retry-loop") {
    parts.push(
      `${rule.evidence.length} edit${rule.evidence.length !== 1 ? "s" : ""}`,
    );
    parts.push("retries");
  } else if (rule.pattern === "tool-mismatch") {
    parts.push(
      `${rule.evidence.length} failed attempt${rule.evidence.length !== 1 ? "s" : ""}`,
    );
  } else if (rule.pattern === "success-pattern") {
    parts.push(
      `${rule.evidence.length} successful edit${rule.evidence.length !== 1 ? "s" : ""}`,
    );
  } else if (rule.pattern === "time-pattern") {
    parts.push(
      `${rule.evidence.length} timed failure${rule.evidence.length !== 1 ? "s" : ""}`,
    );
  }
