...
export async function scan(opts: { force?: boolean; json?: boolean }) {
  ...
  try {
    ...
    spinner.succeed(`Scan complete (${result.files.length} files, ${result.duration}ms)`)
  } catch (err) {
  ...
  if (hasChanges) {
    ...
    try {
      store.replaceRules("scanner", newEntries.flatMap((entry) => entry.rules))
  ...
  if (diff.added.length === 0 && diff.changed.length === 0 && diff.removed.length === 0) {
    console.log(chalk.green("  \u2713") + " No changes detected. Context is up to date.")
  } else {
...
function loadExistingEntries(jsonlPath: string): Map<string, ExistingEntrySummary> {
