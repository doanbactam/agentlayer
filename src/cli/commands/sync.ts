...
export async function sync(opts: {
  ...
  if (opts.remove) {
    for (const name of targets) {
      ...
      if (!adapter) {
        console.error(
          chalk.red(
            `  Unknown tool: ${name}. Available: ${getAdapterNames().join(", ")}`,
          ),
        );
        continue;
      }
      if (!adapter.isInstalled(cwd)) {
        console.log(
          chalk.yellow(`  agentmind context not found in ${adapter.filename}`),
        );
        continue;
      }
      adapter.unsync(cwd);
      console.log(
        chalk.green("  \u2713") +
          ` Removed sync from ${chalk.bold(adapter.filename)}`,
      );
    }
    console.log("");
    return;
  ...
  try {
    ...
    if (health.entries === 0) {
      ...
      console.error(
        chalk.gray("  Run `agentmind scan` to populate the store first.\n"),
      );
      process.exit(1);
    }
    ...
    for (const name of targets) {
      ...
      console.log(
        chalk.green("  \u2713") +
          ` Synced ${chalk.bold(String(health.entries))} entries to ${chalk.cyan(adapter.filename)}`,
      );
    }
  } finally {
    store.close();
