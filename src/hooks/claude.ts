...
function upsertHook(list: ClaudeHook[], hook: ClaudeHook): void {
  ...
  const idx = list.findIndex((h) => h.matcher === hook.matcher && h.hooks.some((cmd) => cmd.includes(file)))
...
function filterAgentmindHooks(list: ClaudeHook[]): ClaudeHook[] {
  return list.filter(
    (h) => !h.hooks.some((hook) => hook.includes("agentmind") || HOOK_MARKERS.some((m) => hook.includes(m)))
  )
}
