...
test("register agent adds it to state", () => {
  ...
  try {
    ...
    assert.ok(
      state.agents["agent-001"].lastHeartbeat,
      "Should have heartbeat timestamp",
    );
  } finally {
    cleanup(dir);
  }
...
test("releaseFiles unlocks files for an agent", () => {
  ...
  try {
    ...
    const state = readState(bridge);
    assert.deepEqual(state.agents["agent-001"].editingFiles, ["src/utils.ts"]);
  } finally {
    cleanup(dir);
  }
});
...
test("whoIsEditing detects conflict when two agents claim same file", () => {
  ...
  try {
    ...
    assert.equal(
      editors.length,
      2,
      "Two agents should be editing the same file",
    );
...
test("getClaimsSnapshot does not mutate inactive state", () => {
  const dir = createTempProject();
  try {
    const bridge = new AgentBridge(dir);
    bridge.register("agent-001", "claude");
    ...
    const before = readFileSync(bridge.getStatePath(), "utf-8");
    const snapshot = bridge.getClaimsSnapshot({ includeInactive: true });
    const after = readFileSync(bridge.getStatePath(), "utf-8");

    assert.equal(
      before,
      after,
      "Reading claims should not rewrite bridge state",
    );
    assert.equal(snapshot.length, 1);
    assert.equal(snapshot[0].claims[0].active, false);
  } finally {
    cleanup(dir);
  }
});

test("getClaimsSnapshot normalizes file paths and filters inactive claims by default", () => {
  const dir = createTempProject();
  try {
    const bridge = new AgentBridge(dir);
    bridge.register("agent-001", "claude");
    ...
    let snapshot = bridge.getClaimsSnapshot({ filePath: "src/index.ts" });
    assert.equal(snapshot.length, 1);
    assert.equal(snapshot[0].filePath, "src/index.ts");
    ...
    snapshot = bridge.getClaimsSnapshot({ filePath: "src/index.ts" });
    assert.deepEqual(
      snapshot,
      [],
      "Inactive claims should be hidden by default",
    );

    snapshot = bridge.getClaimsSnapshot({
