...
test("running scan without init exits with error", () => {
  const dir = createTempDir()
  try {
    // No .agentmind folder - simulate uninitialized project
    const result = runCli(["scan"], dir)
    assert.notEqual(result.status, 0, "scan should fail without init")
    assert.ok(result.stderr.length > 0 || result.stdout.includes("not initialized") || result.stdout.includes("error"), "should have error message")
  } finally {
    cleanup(dir)
  }
})
...
test("running annotate with non-existent file exits with error", () => {
  ...
  try {
    ...
    const result = runCli(["annotate", "non-existent-file.ts", "some annotation"], dir)
    assert.notEqual(result.status, 0, "annotate should fail with non-existent file")
    assert.ok(result.stderr.length > 0 || result.stdout.includes("not found") || result.stdout.includes("error"), "should have error message")
  } finally {
    cleanup(dir)
  }
})
